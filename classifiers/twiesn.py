...
class Classifier_TWIESN:
    ...
    def __init__(self, output_directory, verbose):
        ...
        # hyperparameters 
        first_config = {'N_x': 250, 'connect': 0.5, 'scaleW_in': 1.0, 'lambda': 0.0}
        second_config = {'N_x': 250, 'connect': 0.5, 'scaleW_in': 2.0, 'lambda': 0.05}
        third_config = {'N_x': 500, 'connect': 0.1, 'scaleW_in': 2.0, 'lambda': 0.05}
        fourth_config = {'N_x': 800, 'connect': 0.1, 'scaleW_in': 2.0, 'lambda': 0.05}
    ...
    def init_matrices(self):
        ...
        while not converged:
            ...
            try:
                ...
                eig, _ = linalg.eigs(self.W, k=1, which='LM')
                converged = True
            except:
    ...
    def train(self):
        ...
        # new model 
        ridge_classifier = Ridge(alpha=self._lambda)
    ...
    def fit(self, x_train, y_train, x_test, y_test, y_true):
        ...
        # limit the hyper-parameter search if dataset is too big
        ...
        for idx_config in range(len(self.configs)):
            for rho in self.rho_s:
                ...
                self._lambda = self.configs[idx_config]['lambda']
                self.output_directory = os.path.join(output_directory_root, 'hyper_param_search',
                                                     'config_{}/rho_{}'.format(idx_config, rho))
